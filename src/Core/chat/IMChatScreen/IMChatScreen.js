...
const IMChatScreen = (props) => {
    ...
    navigation.setOptions({
      headerTitle: () => (
        <TouchableOpacity onPress={() => {
          console.log("channelData : " + JSON.stringify(channel))
          if (channel.name) {
  ...
  const onVideoChat = () => {
    console.log("click on video chat")
    ...
    if (channel?.otherParticipants) {
      ...
      // console.log("chanel====>", channel?.otherParticipants,)
      // console.log(channel.id)
      // console.log(channel.name,)
    }


    // props.navigation.navigate('GroupChatVideo', { currentUser: currentUser, channel: channel })
  ...
  const sendMessage = () => {
    ...
    let ciphertext = CryptoJS.AES.encrypt(tempInputValue, 'secretkey123').toString();
  ...
  const resumetoChat = async () => {
    console.log("Resume screen");
    if (global.callOnGoing) {
      IMAudioVideoChat.resumetoChat();
    } else {
      displayCallView;
    }
  }

  const displayCallView = async () => {
    if (global.callOnGoing) {
      return (
        <View style={{ position: 'absolute', zIndex: 999, alignItems: "center", backgroundColor: '#6abd6e', width: "100%", height: 30 }}>
          <TouchableOpacity
            style={{ backgroundColor: '#6abd6e', height: 30, paddingTop: 5 }}
