...
export default class DetailScreen extends React.Component {
  ...
  getData = () => {
    ...
    db.collection("channels").doc(`${channelJson.channelID}`).get().then((snapshot) => {
  ...
  delete(item) {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Yes", onPress: () => { this.onLeaveDecided(item.id) } }
      ]
    );
  }
  ...
  leaveGroup() {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Yes", onPress: () => { this.onExitDecided(currentuser) } }
      ]
    );
  }
  
  onLeaveDecided(itemid) {
    // alert(item.id)
  ...
  renderBody = () => {
    ...
    return <>
      <View style={styles.Container}>
        <Text style={styles.headerTitle2}>{channelJson.name}</Text>
            ...
            <TouchableOpacity onPress={() => { this.setState({ isDialogVisible: true }) }}>
              <View style={{ paddingHorizontal: 20, padding: 20 }}>
                <View style={{ flexDirection: 'row' }}>
                  ...
                  <TouchableOpacity onPress={() => this.props.navigation.navigate('AddMembers', { appStyles: AppStyles, title: "ADD", channel:channelJson })}>
